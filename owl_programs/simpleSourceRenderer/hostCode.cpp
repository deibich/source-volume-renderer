...
namespace {
    ...
    void extractDataForSourceRegions(const std::string &fitsFilePath, box3f &volBox, long voxelsToExtract, std::vector<SourceRegion> & sourceBoxes, std::vector<float> &data)
    {
        fitsfile *fitsFile;
        int fitsError;
        ffopen(&fitsFile, fitsFilePath.c_str(), READONLY, &fitsError);
        assert(fitsError == 0);
...
{
    std::string fileName_originalFits = cmdlineInput.volumeFilePath; 
    std::string fileName_catXML = cmdlineInput.catalogueFilePath;
   
    std::vector<SourceRegion> sourceBoxes;
    auto voxelCount = extractSourceRegionsFromXML(fileName_catXML, sourceBoxes);
    std::vector<float> data;
    extractDataForSourceRegions(fileName_originalFits, volumeBox, voxelCount, sourceBoxes, data);
    ...
    const box3i gridBox(vec3i{volumeBox.lower}, vec3i{volumeBox.upper});
