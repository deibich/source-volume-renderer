...
OPTIX_INTERSECT_PROGRAM(NanoIntersect)()
{
    const auto primID = optixGetPrimitiveIndex();
    const auto& grid = owl::getProgramData<nanovdb::NanoGrid<float>>();
    ...
    nanovdb::Ray<float> ray = { nanovdb::Vec3f{rayOriginObj.x, rayOriginObj.y, rayOriginObj.z} , nanovdb::Vec3f{rayDirectionObj.x, rayDirectionObj.y, rayDirectionObj.z}};
    float t = 0.0f;
    float v0;
    nanovdb::Coord ijk;
    const auto& tree = grid.tree();
    auto acc = tree.getAccessor();
    nanovdb::ZeroCrossing(ray, acc, ijk, v0, t);
